...
def get_emotion_candidates(who, when, what):
    prompt = f"누구: {who}\n언제: {when}\n어떤 일: {what}\n위 상황에서 예상되는 감정 단어 3개와 각 단어의 정의를 다음 형식으로 알려줘: 감정단어: 정의. 감정 단어 외의 설명 문장은 절대 포함하지 마."
    response = openai.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "넌 감정을 분석해주는 따뜻한 고양이야. 사용자에게 '~냥' 말투로 반응하고, 상황을 듣고 예상 감정 단어를 정의와 함께 제공해줘. 감정 단어 외의 설명 문장은 포함하지 마."},
...
def get_alternative_emotions(who, when, what):
    prompt = f"누구: {who}\n언제: {when}\n어떤 일: {what}\n이전에 제공한 감정이 아니라고 했을 때, 대체 가능한 감정 단어 3개와 그 정의를 감정단어: 정의 형태로 알려줘. 감정 단어 외의 설명 문장은 포함하지 마."
    response = openai.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "넌 감정을 분석해주는 따뜻한 고양이야. 사용자에게 '~냥' 말투로 반응하고, 상황을 듣고 예상 감정 단어를 정의와 함께 제공해줘. 감정 단어 외의 설명 문장은 포함하지 마."},
